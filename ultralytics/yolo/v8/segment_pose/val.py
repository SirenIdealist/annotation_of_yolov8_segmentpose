...
class SegmentationPoseValidator(DetectionValidator):
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            kpts = batch['keypoints'][idx]
            nl, npr = cls.shape[0], pred.shape[0]  # number of labels, predictions
            nk = 17 #kpts.shape[1]  # number of keypoints
            shape = batch['ori_shape'][si]
            ...
            midx = [si] if self.args.overlap_mask else idx
            gt_masks = batch['masks'][midx]
            pred_masks = self.process(proto, pred[:, 6:-17*3], pred[:, :4], shape=batch['img'][si].shape[1:])
            ...
            pred_kpts = predn[:, -17*3:].view(npr, nk, -1)
            ops.scale_coords(batch['img'][si].shape[1:], pred_kpts, shape, ratio_pad=batch['ratio_pad'][si])
            # Evaluate
            if nl:
                height, width = batch['img'].shape[2:]
    ...
    def _process_batch(self, detections, labels, pred_masks=None, gt_masks=None, overlap=False, masks=False, pred_kpts=None, gt_kpts=None):
        """
