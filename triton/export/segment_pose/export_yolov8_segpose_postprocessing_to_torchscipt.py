...
class PostprocessingYoloV8MaskPose(torch.nn.Module):
    def __init__(self, conf: float, iou: float, max_det: int, img_h: int, img_w: int, nc: int, output_mask_h: int, 
                 output_mask_w: int, kpt_shape: tuple, max_nms: int, max_wh: int, filtred_classes: List[int]) -> None:
        super(PostprocessingYoloV8MaskPose, self).__init__()
        self.conf = conf
        self.iou = iou
    ...
    def non_max_suppression(self, prediction, proto):
        ...
        nm = prediction.shape[1] - self.nc - 4 - self.kpt_shape[0] * self.kpt_shape[1]
        mi = 4 + self.nc

        xc = prediction[:, 4:mi].amax(1) > self.conf  # candidates

        boxes_output = torch.zeros(bs, self.max_det, 4, device=device)
        scores_output = torch.zeros(bs, self.max_det, 1, device=device)
        label_output = torch.zeros(bs, self.max_det, 1, device=device)
        kpts_output = torch.zeros(bs, self.max_det, 51, device=device)
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            box, cls, mask, kpts = x.split((4, self.nc, nm, 17*3), 1)
            ...
            kpts_output[xi, :top_k] = kpts[indexes]
        return boxes_output, scores_output, label_output, masks_output_global, kpts_output

    def forward(self, preds: torch.Tensor, proto: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        boxes, scores, label, masks, kpts = self.non_max_suppression(preds, proto)
        return boxes, scores, label, masks, kpts.view(preds.shape[0], self.max_det, *self.kpt_shape)


class OnnxInferenceYoloSegPose:
...
def main(args):
    yolo_seg_model = OnnxInferenceYoloSegPose(args.path_onnx_file)
    ...
    yolo_output_preds, yolo_output_proto = yolo_seg_model(image_list_resized)

    postproc_model = PostprocessingYoloV8MaskPose(
        conf=args.conf,
        iou=args.iou,
    ...
    batch_boxes, batch_scores, batch_labels, batch_masks, batch_kpts = decode_detection(
        detections)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'Export postprocessing for instance segmentation_pose to torchscipt')
    ...
    parser.add_argument('--path_output_model',
                        default='triton/deploy/model_repository/yolov8_segpose_postprocessing/1/model.pt', type=str)
    parser.add_argument(
        '--img_dir', default='./ultralytics/assets', type=str)
    # $ yolo export model=yolov8m-segpose.pt format=onnx dynamic=True
    parser.add_argument('--path_onnx_file',
                        default='./yolov8m-segpose.onnx', type=str)
